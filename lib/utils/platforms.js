...
function detectPlatform(platform) {
    ...
    if (_.contains(name, 'linux')
        ...
        else if (hasSuffix(name, '.tgz') || hasSuffix(name, '.tar.gz')) {
    ...
    suffix = suffix || '64'; // (prefix == platforms.OSX? '64' : '32');
...
function satisfiesPlatform(platform, list) {
    ...
    // Handle github's special '64' type
    if (_.contains(list, '64')) return hasSuffix(platform, '_64');
...
function resolveForVersion(version, platformID, opts) {
    ...
    return _.chain(version.platforms)
        .filter(function(pl) {
            return pl.type.indexOf(platformID) === 0 || pl.type === '64' && hasSuffix(platformID, '_64');
        })
        .sort(function(p1, p2) {
            var result = 0;
